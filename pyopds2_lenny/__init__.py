...
def _unwrap_search_response(resp):
    ...
    Accepts tuple-like (records, total), objects with common attributes
    ('records','docs','items','data'), or any iterable. Returns (records, total).
    """
    ...
    for rec_attr in ("records", "docs", "items", "data"):
        if hasattr(resp, rec_attr):
            records = getattr(resp, rec_attr)
            total = None
...
class LennyDataProvider(OpenLibraryDataProvider):
    ...
    def search(
        ...
        # The upstream provider may return different shapes (tuple, object,
        # or iterable). Normalize into (records, total).
